...
# clust_X = wifi_mean.ix[:, 0 : ]
# #clust_X = max_abs_scaler.fit_transform(clust_X)
# clust_X = normalize.fit_transform(clust_X)
# clus.fit(clust_X)
...
for k in range(0, clus_num):
    ...
    if k != 0:
        ...
        if score.mean() < -6:
            print('score less than -6, use mean!')
            res_t = wifi_mean.ix[index, 553 : 553 + 18].as_matrix()
        ...
        if score.mean() < -6:
            print('score less than -6, use mean!')
            res = wifi_mean.ix[index, 553 : 553 + 18]
...
sub.to_csv('sub10_Mix_MultiElastic_and_rule.csv', index = False)