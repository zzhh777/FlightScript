...
model.fit({'G_input' : train_X[:, 0], 'AP_input' : train_X[:, 1 : 750], 'his_input' : train_X[:, 750 :]}, train_Y,\
    batch_size=60, nb_epoch=200, shuffle = True)
#model.fit({'G_input' : train_X[:, 0], 'AP_input' : train_X[:, 1 : 750], 'his_input' : train_X[:, 750 :]}, train_Y,\
#    batch_size=60, nb_epoch=200, validation_split = 0.2, shuffle = True)
...
X = np.append(np.tile(wifi_t.ix[:, i - 1].T, (749, 1)), X, axis = 1)
X = np.append(wifi_t.ix[:, 'group'].reshape([749, 1]), X, axis = 1)
res = model.predict({'G_input' : X[:, 0], 'AP_input' : X[:, 1 : 750], 'his_input' : X[:, 750 :]},  batch_size=749)
print(res.shape)
...
sub.to_csv('sub6_keras.csv', index = False)