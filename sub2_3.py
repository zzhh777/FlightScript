...
#link_slice_mean['WIFIAPTag'] = link_slice_10['WIFIAPTag']
#link_slice_10 = link_slice_10[link_slice_10['WIFIAPTag'].apply(lambda x : x not in miss)]
#link_slice_mean = link_slice_mean[link_slice_mean['WIFIAPTag'].apply(lambda x : x not in miss)]

#link_slice_mean = link_slice_mean.drop(['WIFIAPTag'], axis = 1)
...
#采集训练数据，每200分钟采集一次，
print('getting data...')
for i in [x for x in range(1470, 1950) if (x % 1 == 0) & (x != 0)]:
    #print(i)
    if i not in range(2100, 2130):
        ...
        if i != 1470:
            ...
            #data_t = np.append(slide_mean_1.ix[:, i : i + 17], data_t, axis = 1)
            ...
            #data = np.append(slide_mean_1.ix[:, i : i + 17], data, axis = 1)
...
#real_test = np.append(mean_One_order_diff_temp.ix[:, i % 144 : last_i], real_test, axis = 1)
#real_test = np.append(slide_mean_1.ix[:, i : i + 17], real_test, axis = 1)
